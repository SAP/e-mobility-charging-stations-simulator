...
function createTestableWorkerBroadcastChannel (
  ...
  return {
    ...
    requestParams: testable.requestParams,
  }
}
...
await describe('ChargingStationWorkerBroadcastChannel', async () => {
  ...
  await describe('STOP_TRANSACTION handler', async () => {
    await it('should preserve valid UI overrides and transport params while using canonical transaction identity and meterStop', async () => {
      ...
      const {
        ...
        onTransportError,
        ...requestOptions
      } = stopCall.arguments[3] as RequestParams
  ...
  await describe('METER_VALUES handler delivery accounting', async () => {
    const runGeneratedMeterValuesFailure = async (
      ...
      if (testable.requestParams.onMessageSent != null) {
        const callback = testable.requestParams.onMessageSent
        testable.requestParams.onMessageSent = () => {
          relayed()
          callback()
        }
      }
      if (testable.requestParams.onTransportError != null) {
        const callback = testable.requestParams.onTransportError
        testable.requestParams.onTransportError = (error, deliveryAmbiguous) => {
          relayed()
          callback(error, deliveryAmbiguous)
        }
      }

      await assert.rejects(
        testable.commandHandler(BroadcastChannelProcedureName.METER_VALUES, { connectorId: 1 }),
        error => error === failure
      )
      if (deliveredParams == null) throw new Error('MeterValues request was not delivered')
    ...
    await it('should restore generated interval energy after a certain pre-send failure', async () => {
      const result = await runGeneratedMeterValuesFailure(
        ...
        params => {
          params.onTransportError = () => undefined
        }
      )

      assert.deepStrictEqual(result, { carry: 10, keyReserved: false, relayedCalls: 1 })
    })

    await it('should preserve ambiguous delivery consumption and restore confirmed CALLERROR', async () => {
      const sent = await runGeneratedMeterValuesFailure(
        params => params.onMessageSent?.(),
        params => {
          params.onMessageSent = () => undefined
        }
      )
      const ambiguous = await runGeneratedMeterValuesFailure(
        (params, failure) => params.onTransportError?.(failure, true),
        params => {
          params.onTransportError = () => undefined
        }
      )
      const callError = await runGeneratedMeterValuesFailure(
        (params, failure) => {
      ...
      assert.deepStrictEqual(sent, { carry: 0, keyReserved: true, relayedCalls: 1 })
      assert.deepStrictEqual(ambiguous, { carry: 0, keyReserved: true, relayedCalls: 1 })
      assert.deepStrictEqual(callError, { carry: 10, keyReserved: false, relayedCalls: 1 })
    })

    await it('should restore a buffered generated interval after replay returns CALLERROR', async () => {
      const result = await runGeneratedMeterValuesFailure(
        params => params.onRequestBuffered?.(),
        undefined,
    ...
    await it('should resolve OCPP 2.0 OncePerTransaction ownership by delivery certainty', async () => {
      const failure = new OCPPError(ErrorType.GENERIC_ERROR, 'MeterValues failed')
      let keyReservedAtDispatch = false
      const requestHandler = mock.fn((...args: unknown[]): Promise<unknown> => {
        keyReservedAtDispatch =
          station.getConnectorStatus(1, 1)?.publicKeySentInTransaction === true
      ...
      const { station } = createMockChargingStation({
        ...
        stationInfo: { meterSerialNumber: 'SIM-001', ocppVersion: OCPPVersion.VERSION_201 },
      })
      ...
      assert.strictEqual(keyReservedAtDispatch, true)
      assert.strictEqual(connectorStatus.publicKeySentInTransaction, false)
    ...
    await it('should settle a signed OCPP 2.0 broadcast before building Ended', async () => {
      const broadcastStarted = Promise.withResolvers<undefined>()
      const releaseBroadcast = Promise.withResolvers<undefined>()
      let broadcastPublicKey: string | undefined
      let endedPublicKey: string | undefined
      let endedSent = false
      const requestHandler = mock.fn(async (...args: unknown[]): Promise<unknown> => {
        const command = args[1] as RequestCommand
        const params = args[3] as RequestParams
        if (command === RequestCommand.METER_VALUES) {
          const payload = args[2] as {
            meterValue: { sampledValue: { signedMeterValue?: { publicKey?: string } }[] }[]
          }
          broadcastPublicKey = payload.meterValue[0].sampledValue.find(
        ...
        const payload = args[2] as { eventType?: OCPP20TransactionEventEnumType }
        if (payload.eventType === OCPP20TransactionEventEnumType.Ended) {
          const endedPayload = args[2] as {
            meterValue?: { sampledValue: { signedMeterValue?: { publicKey?: string } }[] }[]
          }
          endedPublicKey = endedPayload.meterValue
            ?.flatMap(meterValue => meterValue.sampledValue)
      ...
      const broadcast = testable.commandHandler(BroadcastChannelProcedureName.METER_VALUES, {
        connectorId: 1,
        evseId: 1,
      })
      await broadcastStarted.promise
      const stop = OCPP20ServiceUtils.requestStopTransaction(station, 1, 1)
      await flushMicrotasks()

      assert.strictEqual(endedSent, false)
      releaseBroadcast.resolve(undefined)
      await Promise.all([broadcast, stop])
      assert.strictEqual(endedSent, true)
      assert.strictEqual(typeof broadcastPublicKey, 'string')
      assert.notStrictEqual(broadcastPublicKey, '')
      assert.strictEqual(endedPublicKey, '')
    })

    await it('should reject a malformed OCPP 2.0.1 meterValue override before dispatch', async () => {
      const { sentRequests, station } = createMockStationWithRequestTracking()
      instance = new ChargingStationWorkerBroadcastChannel(station)
      const testable = createTestableWorkerBroadcastChannel(instance)
      const handler = testable.commandHandlers.get(BroadcastChannelProcedureName.METER_VALUES)
      assert.ok(handler != null)

      await assert.rejects(
        async () => await handler({ connectorId: 1, meterValue: {} }),
        /meterValue must be an array/
      )
      assert.strictEqual(sentRequests.length, 0)
    })
  })

...
await describe('J01 - Autonomous clock-aligned MeterValues (#2011 Category 2F)', async () => {
  ...
  await describe('emitClockAlignedMeterValues (per-tick sweep)', async () => {
    ...
    await it('should not emit restored transaction data ahead of its durable replay backlog', async () => {
      const { mockStation, requestHandlerMock } = createAlignedStation({
        connectorsCount: 1,
        evsesCount: 1,
      })
      ...
      connectorStatus.transactionEventQueue = [
        {
          request: {
            ...
            transactionInfo: { transactionId: '00000000-0000-4000-8000-000000000012' },
    ...
    await it('isolates idle EVSE and station interval rollback', async () => {
      ...
      connectorStatus.energyActiveImportIntervalBaselines = {
        [stationBaselineKey]: 5,
      }
      let failEvseRequest = false
      requestHandlerMock.mock.mockImplementation((...args: unknown[]): Promise<unknown> => {
        ...
        if (
          args[1] === OCPP20RequestCommand.METER_VALUES &&
          request.evseId === 1 &&
          failEvseRequest
        ) {
          return Promise.reject(
            new OCPPError(
              ...
              'EVSE MeterValues request rejected',
      ...
      await OCPP20ServiceUtils.emitClockAlignedMeterValues(mockStation, new Date(0))
      connectorStatus.energyActiveImportRegisterValue = 40
      failEvseRequest = true
      requestHandlerMock.mock.resetCalls()

      await OCPP20ServiceUtils.emitClockAlignedMeterValues(mockStation, new Date(60_000))

      assert.strictEqual(
        connectorStatus.energyActiveImportIntervalBaselines[stationBaselineKey],
        40
      )
      assert.strictEqual(evseStatus.energyActiveImportIntervalBaseline, 10)
      const firstStationInterval = sentPayloads(requestHandlerMock)
        .find(({ evseId }) => evseId === 0)
        ?.meterValue.flatMap(meterValue => meterValue.sampledValue)
        .find(
          sample => sample.measurand === OCPP20MeasurandEnumType.ENERGY_ACTIVE_IMPORT_INTERVAL
        )?.value
      assert.strictEqual(firstStationInterval, 30)

      failEvseRequest = false
      requestHandlerMock.mock.resetCalls()
      await OCPP20ServiceUtils.emitClockAlignedMeterValues(mockStation, new Date(120_000))

      const secondIntervals = new Map(
        sentPayloads(requestHandlerMock).map(payload => [
          payload.evseId,
          payload.meterValue
            .flatMap(meterValue => meterValue.sampledValue)
            .find(
      ...
      assert.strictEqual(secondIntervals.get(0), 0)
      assert.strictEqual(secondIntervals.get(1), 30)
      assert.strictEqual(evseStatus.energyActiveImportIntervalBaseline, 40)
    })
    ...
    await it('restores a graceful pre-send failure classified as definitely unsent', async () => {
      ...
      connectorStatus.energyActiveImportIntervalBaselines = { [baselineKey]: 5 }
      let stopping = false
      mock.method(mockStation, 'isStopping', () => stopping)
      requestHandlerMock.mock.mockImplementation((...args: unknown[]): Promise<unknown> => {
        const request = args[2] as OCPP20MeterValuesRequest
        const requestParams = args[3] as RequestParams | undefined
        if (request.evseId === 0) {
    ...
    await it('does not restore an in-flight aligned baseline when graceful stop cancels its pending send', async () => {
      ...
      station.stationInfo.meteringPerTransaction = false
      ...
      const stationEvseStatus = station.getEvseStatus(0)
    ...
    await it('should start a new drain when a request arrives before the prior finalizer', async () => {
      const { mockStation, requestHandlerMock } = createAlignedStation({
        connectorsCount: 1,
        evsesCount: 1,
      })
      const firstDelivery = Promise.withResolvers<unknown>()
      requestHandlerMock.mock.mockImplementationOnce(() => firstDelivery.promise)
      const request = (timestamp: number): OCPP20MeterValuesRequest => ({
        evseId: 1,
        meterValue: [{ sampledValue: [], timestamp: new Date(timestamp) }],
      })

      const first = OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(
        mockStation,
        1,
        request(60_000)
      )
      const second = firstDelivery.promise.then(() =>
        OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(mockStation, 1, request(120_000))
      )
      ...
      assert.deepStrictEqual(
        sentPayloads(requestHandlerMock).map(payload => payload.meterValue[0].timestamp.getTime()),
        [60_000, 120_000]
      )
    })
    ...
    await it('settles a cancelled pending trigger exactly once and releases its barrier', async () => {
      ...
      setupConnectorWithTransaction(mockStation, 1, { transactionId: 'tx-cancel-pending' })
      const connectorStatus = mockStation.getConnectorStatus(1, 1)
      assert.ok(connectorStatus != null)
      const firstRequest = Promise.withResolvers<unknown>()
      requestHandlerMock.mock.mockImplementationOnce(() => firstRequest.promise)
      const request = (timestamp: number): OCPP20MeterValuesRequest => ({
        evseId: 1,
        meterValue: [{ sampledValue: [], timestamp: new Date(timestamp) }],
      })
      const inFlight = OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(
        mockStation,
        1,
        request(60_000)
      )
      assert.strictEqual(
        OCPP20ServiceUtils.reserveTriggeredMeterValuesRequests(mockStation, [1]),
        true
      )
      ...
      await OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(
        mockStation,
        1,
        request(120_000),
        undefined,
        [{ key: connectorStatus, restore: () => restoreCount++ }],
        true,
        [],
        definitivelyRejected => {
          settlements.push(definitivelyRejected)
          delivery.settle(definitivelyRejected)
        }
      )

      OCPP20ServiceUtils.cancelClockAlignedMeterValuesRequests(mockStation)
      OCPP20ServiceUtils.cancelClockAlignedMeterValuesRequests(mockStation)

      assert.deepStrictEqual(settlements, [true])
      assert.strictEqual(restoreCount, 1)
      const successor = TransactionMeterValueDeliveryBarrier.begin(
        connectorStatus,
        'tx-cancel-pending'
      )
      assert.ok(successor != null)
      assert.strictEqual(successor.waitForTurn(), undefined)
      successor.settle()
    ...
    await it('should send only triggered aligned work while registration is pending', async () => {
      const { mockStation, requestHandlerMock } = createAlignedStation({
        connectorsCount: 1,
        evsesCount: 1,
      })
      mockStation.inAcceptedState = () => false
      mockStation.inPendingState = () => true
      ...
      await OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(mockStation, 1, request(120_000))

      assert.strictEqual(requestHandlerMock.mock.callCount(), 1)
      const requestParams = requestHandlerMock.mock.calls[0].arguments[3] as RequestParams
      assert.strictEqual(requestParams.triggerMessage, true)
    })

    await it('should preserve a reserved trigger and restore a later autonomous candidate', async () => {
      const { mockStation, requestHandlerMock } = createAlignedStation({
        connectorsCount: 1,
        evsesCount: 1,
      })
      const releaseFirst = Promise.withResolvers<undefined>()
      requestHandlerMock.mock.mockImplementationOnce(async () => {
      ...
      await OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(
        ...
        request(180_000),
        undefined,
        [{ consumed: 7, key: {}, restore: consumed => (restored += consumed) }]
      )

      assert.strictEqual(restored, 7)
      releaseFirst.resolve(undefined)
      await first

      assert.strictEqual(requestHandlerMock.mock.callCount(), 2)
      const requestParams = requestHandlerMock.mock.calls[1].arguments[3] as RequestParams
      assert.strictEqual(requestParams.triggerMessage, true)
      assert.strictEqual(
        sentPayloads(requestHandlerMock)[1].meterValue[0].timestamp.getTime(),
        120_000
      )
    })

    await it('should reject an atomic trigger reservation behind autonomous pending work', async () => {
      const { mockStation, requestHandlerMock } = createAlignedStation({
        connectorsCount: 1,
        evsesCount: 1,
      })
      ...
      assert.strictEqual(requestHandlerMock.mock.callCount(), 2)
      const requestParams = requestHandlerMock.mock.calls[1].arguments[3] as RequestParams
      assert.strictEqual(requestParams.triggerMessage, false)
      assert.strictEqual(
        sentPayloads(requestHandlerMock)[1].meterValue[0].timestamp.getTime(),
        120_000
      )
    ...
    await it('should retry a failed reserved trigger exactly and restore later autonomous work', async () => {
      const { mockStation, requestHandlerMock } = createAlignedStation({
        connectorsCount: 1,
        evsesCount: 1,
      })
      ...
      requestHandlerMock.mock.mockImplementationOnce(async () => {
        ...
        throw new Error('definitely unsent')
      })
      const request = (timestamp: number): OCPP20MeterValuesRequest => ({
        evseId: 1,
        meterValue: [{ sampledValue: [], timestamp: new Date(timestamp) }],
      ...
      assert.strictEqual(requestHandlerMock.mock.callCount(), 2)
      const retriedParams = requestHandlerMock.mock.calls[1].arguments[3] as RequestParams
      assert.strictEqual(retriedParams.triggerMessage, true)
      assert.strictEqual(restored, 7)
      assert.deepStrictEqual(
        sentPayloads(requestHandlerMock)[1].meterValue.map(value => value.timestamp.getTime()),
        [60_000]
      )
    })

    await it('should reject later signed evidence rather than coalescing exact requests', async () => {
      const { mockStation, requestHandlerMock } = createAlignedStation({
        connectorsCount: 1,
        evsesCount: 1,
      })
      ...
      const request = (timestamp: number, signedMeterData: string): OCPP20MeterValuesRequest => ({
        evseId: 1,
        meterValue: [
          {
            sampledValue: [
      ...
      const first = OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(
        mockStation,
        1,
        request(60_000, 'first')
      )
      await OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(
        mockStation,
        1,
        request(120_000, 'preserved')
      )
      let restored = 0
      await OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(
        mockStation,
        1,
        request(180_000, 'latest'),
        undefined,
        [{ consumed: 7, key: {}, restore: consumed => (restored += consumed) }]
      )
      assert.strictEqual(restored, 7)
      ...
      const coalescedPayload = sentPayloads(requestHandlerMock)[1]
      assert.deepStrictEqual(
        coalescedPayload.meterValue.flatMap(meterValue =>
        ...
        [{ signedMeterData: 'preserved', timestamp: 120_000 }]
      )
    })

    for (const exactKind of ['signed', 'trigger'] as const) {
      await it(`should reject a trigger reservation behind an in-flight ${exactKind} request`, async () => {
        const { mockStation, requestHandlerMock } = createAlignedStation({
          connectorsCount: 1,
          evsesCount: 1,
        })
        const firstResponse = Promise.withResolvers<undefined>()
        requestHandlerMock.mock.mockImplementationOnce(async (): Promise<unknown> => {
        ...
        if (exactKind === 'trigger') {
          assert.strictEqual(
            OCPP20ServiceUtils.reserveTriggeredMeterValuesRequests(mockStation, [1]),
            true
          )
        }
        const firstDrain = OCPP20ServiceUtils.sendClockAlignedMeterValuesRequest(
          mockStation,
          1,
          request,
          undefined,
    ...
    await it('should preserve an accepted trigger behind a definitely-unsent unsigned request', async () => {
      const { mockStation, requestHandlerMock } = createAlignedStation({
        connectorsCount: 1,
        evsesCount: 1,
      })
      ...
      assert.deepStrictEqual(
        requestHandlerMock.mock.calls.map(
          call => (call.arguments[3] as RequestParams).triggerMessage
        ),
        [false, true]
      )
    })
    ...
    await it('preserves interval energy while aligned requests are coalesced', async () => {
      ...
      assert.strictEqual(stationPayloads.length, 3)
      const intervalValues = stationPayloads
        .slice(1)
        .map(payload =>
          payload.meterValue[0].sampledValue
            ...
            .map(sample => [sample.unitOfMeasure?.unit, sample.value])
        )
      assert.deepStrictEqual(intervalValues, [
        [
          ['Wh', 40],
          ['kWh', 0.04],
        ],
    ...
    await it('isolates a connector build failure from the remaining EVSEs', () => {
      ...
      assert.strictEqual(failedEvseStatus.energyActiveImportIntervalBaseline, 95)
      assert.strictEqual(
        failedConnectorStatus.energyActiveImportIntervalBaselines[stationBaselineKey],
        100
      )
    })
    ...
    await it('should not sign active aligned samples when standard SignReadings is disabled', async () => {
      const { mockStation, requestHandlerMock } = alignedStation
      upsertConfigurationKey(mockStation, ALIGNED_DATA_INTERVAL_KEY, '60')
      upsertConfigurationKey(mockStation, ALIGNED_ENABLED_KEY, 'true')
      upsertConfigurationKey(mockStation, SEND_DURING_IDLE_KEY, 'false')
      upsertConfigurationKey(mockStation, SIGN_READINGS_KEY, 'false')
      upsertConfigurationKey(mockStation, SIGN_UPDATED_READINGS_KEY, 'true')
      upsertConfigurationKey(mockStation, FISCAL_PUBLIC_KEY, TEST_PUBLIC_KEY_HEX)
      upsertConfigurationKey(
        mockStation,
      ...
      setupConnectorWithTransaction(mockStation, 1, { transactionId: 'tx-sign-vendor' })
    ...
    await it('releases a builder-owned key after final CALLERROR', async () => {
      ...
      setupConnectorWithTransaction(mockStation, 1, { transactionId: 'tx-final-callerror' })
      const connectorStatus = mockStation.getConnectorStatus(1)
      assert.ok(connectorStatus != null)
      connectorStatus.transactionEnergyActiveImportRegisterValue = 1234
      let reject = true
    ...
    await it('compacts older Ended meter data before touching a protected Ended event', () => {
          ...
          sampledValue: [
            {
              ...(index === 0 || index === 239
                ...
                : { measurand: OCPP20MeasurandEnumType.POWER_ACTIVE_IMPORT }),
              customData: { payload: 'x'.repeat(2400), vendorId: 'test' },
              value: index,
            },
          ],
      ...
      assert.strictEqual(
        retainedEvent.request.meterValue[0].sampledValue[0].customData?.payload,
        'x'.repeat(2400)
      )
      assert.strictEqual(
      ...
      assert.strictEqual(
        retainedEvent.request.meterValue[120].sampledValue[0].customData?.payload,
        'x'.repeat(2400)
      )

...
await describe('OCPP16ServiceUtils — pure functions', async () => {
  ...
  await describe('stopTransactionOnConnector', async () => {
    ...
    const runPeriodicMeterValuesFailure = async (
    ...
    ): Promise<{ carry: number; keyCounts: number[]; keyReserved: boolean }> => {
      const failure = new OCPPError(ErrorType.GENERIC_ERROR, 'MeterValues failed')
      const payloads: { meterValue: OCPP16MeterValue[] }[] = []
      const requestHandler = mock.fn((...args: unknown[]): Promise<unknown> => {
        payloads.push(args[2] as { meterValue: OCPP16MeterValue[] })
      ...
      tickInterval()
      await flushMicrotasks()
      const timer = connectorStatus.transactionUpdatedMeterValuesSetInterval
      assert.ok(timer != null)
      clearInterval(timer)

      const keyCounts = payloads.map(
        payload =>
          payload.meterValue
            .flatMap(meterValue => meterValue.sampledValue)
            .filter(sampledValue => {
              if (sampledValue.format !== OCPP16MeterValueFormat.SIGNED_DATA) return false
              const publicKey = (JSON.parse(sampledValue.value) as { publicKey?: string }).publicKey
    ...
    await it('should restore energy and retry the key after certain periodic failures', async t => {
      t.mock.timers.enable({ apis: ['setInterval'] })

      for (const configureDelivery of [
        (params: RequestParams, failure: OCPPError) => params.onTransportError?.(failure, false),
    ...
    await it('should settle periodic interval rollback before building StopTransaction', async t => {
      t.mock.timers.enable({ apis: ['setInterval'] })
      const periodicResponse = Promise.withResolvers<unknown>()
      let periodicRequestParams: RequestParams | undefined
      let carryAtStopDispatch: number | undefined
      let stopRequestCount = 0
      const requestHandler = mock.fn((...args: unknown[]): Promise<unknown> => {
        if (args[1] === OCPP16RequestCommand.METER_VALUES) {
          periodicRequestParams = args[3] as RequestParams
          periodicRequestParams.onMessageSent?.()
          return periodicResponse.promise
        }
        if (args[1] === OCPP16RequestCommand.STOP_TRANSACTION) {
          carryAtStopDispatch =
            station.getConnectorStatus(1)?.transactionEnergyActiveImportIntervalCarry?.default
          stopRequestCount++
      ...
      t.mock.timers.tick(1000)
      await flushMicrotasks()
      assert.ok(periodicRequestParams != null)

      const stop = OCPP16ServiceUtils.stopTransactionOnConnector(station, 1)
      await flushMicrotasks()
      assert.strictEqual(stopRequestCount, 0)

      const callError = new OCPPError(ErrorType.GENERIC_ERROR, 'periodic CALLERROR')
      periodicRequestParams.onError?.(callError, true)
      periodicResponse.reject(callError)
      await stop

      assert.strictEqual(stopRequestCount, 1)
      assert.strictEqual(carryAtStopDispatch, 1244)
    })

    await it('should stop the periodic producer before draining an in-flight sample', async t => {
      t.mock.timers.enable({ apis: ['setInterval'] })
      const periodicResponse = Promise.withResolvers<unknown>()
      let meterValuesRequests = 0
      let stopRequests = 0
      const requestHandler = mock.fn((...args: unknown[]): Promise<unknown> => {
        if (args[1] === OCPP16RequestCommand.METER_VALUES) {
          meterValuesRequests++
        ...
        if (args[1] === OCPP16RequestCommand.STOP_TRANSACTION) stopRequests++
        return Promise.resolve({ idTagInfo: { status: OCPP16AuthorizationStatus.ACCEPTED } })
      })
      const { station } = createMockChargingStation({
        ocppRequestService: { requestHandler },
      ...
      OCPP16ServiceUtils.startUpdatedMeterValues(station, 1, 1000)
      t.mock.timers.tick(1000)
      assert.strictEqual(meterValuesRequests, 1)
      ...
      t.mock.timers.tick(1000)
      assert.strictEqual(meterValuesRequests, 1)
      assert.strictEqual(stopRequests, 0)

      periodicResponse.resolve({})
      await stop
      assert.strictEqual(stopRequests, 1)
    })

    await it('should queue StopTransaction behind a buffered periodic sample and reconcile a rejected predecessor', async t => {
      t.mock.timers.enable({ apis: ['setInterval'] })
      const bufferedReplay = Promise.withResolvers<unknown>()
      const bufferedStop = Promise.withResolvers<unknown>()
      const stopRequestStarted = Promise.withResolvers<undefined>()
      let periodicRequestParams: RequestParams | undefined
      let stopRequestParams: RequestParams | undefined
      let stopRequests = 0
      const requestHandler = mock.fn((...args: unknown[]): Promise<unknown> => {
        if (args[1] === OCPP16RequestCommand.METER_VALUES) {
          periodicRequestParams = args[3] as RequestParams
          periodicRequestParams.onRequestBuffered?.()
          return bufferedReplay.promise
        }
        if (args[1] === OCPP16RequestCommand.STOP_TRANSACTION) {
          stopRequests++
          stopRequestStarted.resolve(undefined)
          stopRequestParams = args[3] as RequestParams
          stopRequestParams.onRequestBuffered?.()
          return bufferedStop.promise
        }
        return Promise.resolve({})
      })
      const { station } = createMockChargingStation({
      ...
      const replacePayload = mock.fn(() => true)
      ;(
        station as unknown as {
          replaceBufferedRequestPayload: typeof replacePayload
        }
      ).replaceBufferedRequestPayload = replacePayload
      ...
      assert.strictEqual(stopRequests, 1)
      assert.strictEqual(stopRequestParams?.bufferWithoutSending, true)
      assert.strictEqual(connectorStatus.transactionUpdatedMeterValuesSetInterval, undefined)

      const callError = new OCPPError(ErrorType.GENERIC_ERROR, 'periodic CALLERROR')
      periodicRequestParams.onError?.(callError, true)
      bufferedReplay.reject(callError)
      await flushMicrotasks()

      assert.strictEqual(replacePayload.mock.callCount(), 1)
      const [commandName, previousPayload, replacementPayload] = replacePayload.mock.calls[0]
      ...
      const countPublicKeys = (payload: OCPP16StopTransactionRequest): number =>
        payload.transactionData
          ?.flatMap(meterValue => meterValue.sampledValue)
          .filter(sampledValue => {
            if (sampledValue.format !== OCPP16MeterValueFormat.SIGNED_DATA) return false
            const signedValue = JSON.parse(sampledValue.value) as { publicKey?: string }
            return typeof signedValue.publicKey === 'string' && signedValue.publicKey.length > 0
          }).length ?? 0
    ...
    await it('should reject malformed nested transactionData before stop state mutates', async () => {
      ...
      OCPP16ServiceUtils.startUpdatedMeterValues(station, 1, 60_000)
      ...
      try {
        ...
        assert.strictEqual(
          Object.hasOwn(connectorStatus, 'transactionUpdatedMeterValuesSetInterval'),
          true
        )
    ...
    await it('should force strict StopTransaction errors for every joiner and clear the failed operation', async () => {
      ...
      const requestHandler = mock.fn((...args: unknown[]) => {
        ...
        const {
          onError,
          onMessageSent: wrappedOnMessageSent,
          onRequestBuffered,
          onResponseReceived,
          onTransportError,
          ...requestOptions
    ...
    await it('should restore the periodic producer after a retryable stop failure', async () => {
      ...
      const requestHandler = mock.fn((...args: unknown[]): Promise<unknown> => {
        if (args[1] === OCPP16RequestCommand.STOP_TRANSACTION) {
          stopAttempts++
          if (stopAttempts === 1) {
            ;(args[3] as RequestParams).onError?.(sendFailure as OCPPError, false)
      ...
      const meterValuesTimer = connectorStatus.transactionUpdatedMeterValuesSetInterval
      assert.ok(meterValuesTimer != null)
      const startUpdatedMeterValues =
        OCPP16ServiceUtils.startUpdatedMeterValues.bind(OCPP16ServiceUtils)
      const restorePeriodicMeterValues = mock.method(
        OCPP16ServiceUtils,
        'startUpdatedMeterValues',
        (targetStation: ChargingStation, targetConnectorId: number, interval: number) => {
          startUpdatedMeterValues(targetStation, targetConnectorId, interval)
        }
      )
      ...
      try {
        ...
        assert.strictEqual(restorePeriodicMeterValues.mock.callCount(), 1)
        assert.strictEqual(
          Object.hasOwn(connectorStatus, 'transactionUpdatedMeterValuesSetInterval'),
          true
        )
    ...
    await it('should transfer a rejected strict end key without mutating the attempted payload', async () => {
      const callError = new OCPPError(ErrorType.GENERIC_ERROR, 'MeterValues CALLERROR')
      let meterValuesPayload: undefined | { meterValue: OCPP16MeterValue[] }
      let attemptedMeterValuesBytes: string | undefined
      let stopPayload: OCPP16StopTransactionRequest | undefined
      const requestHandler = mock.fn((...args: unknown[]): Promise<unknown> => {
        if (args[1] === OCPP16RequestCommand.METER_VALUES) {
          meterValuesPayload = args[2] as { meterValue: OCPP16MeterValue[] }
          attemptedMeterValuesBytes = JSON.stringify(meterValuesPayload)
          const params = args[3] as RequestParams
          params.onMessageSent?.()
          params.onError?.(callError, true)
      ...
      assert.ok(meterValuesPayload != null)
      assert.strictEqual(JSON.stringify(meterValuesPayload), attemptedMeterValuesBytes)
      assert.ok(stopPayload?.transactionData != null)
      const meterValuesSignedData = meterValuesPayload.meterValue[0].sampledValue
        .filter(sampledValue => sampledValue.format === OCPP16MeterValueFormat.SIGNED_DATA)
        .map(
          sampledValue =>
            JSON.parse(sampledValue.value) as {
              publicKey: string
              signedMeterData: string
            }
        )
        .find(signedData => signedData.publicKey.length > 0)
      const stopSignedData = stopPayload.transactionData
        .flatMap(meterValue => meterValue.sampledValue)
        .filter(sampledValue => sampledValue.format === OCPP16MeterValueFormat.SIGNED_DATA)
        .map(
          sampledValue =>
        ...
        .find(signedData => signedData.publicKey.length > 0)
      assert.ok(meterValuesSignedData != null)
      assert.ok(stopSignedData != null)
      assert.strictEqual(stopSignedData.publicKey, meterValuesSignedData.publicKey)
      assert.strictEqual(stopSignedData.signedMeterData, meterValuesSignedData.signedMeterData)
      assert.strictEqual(connectorStatus.publicKeySentInTransaction, true)
    })

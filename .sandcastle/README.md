...
**Empty-arbiter guard**: when the arbiter returns parseable but empty findings while the merged list was non-empty, the merge result is preserved (treated as soft parse failure with a warning). Only triggers when the arbiter actually fired (i.e. merged contained HIGH/CRITICAL); legitimate dismissal of a non-HIGH/CRITICAL merge does not reach this path.
...
**Family diversity (PoLL constraint)**: bias reduction in heterogeneous panels hinges on model-family diversity, not just slot count. PoLL ([Verga et al. 2024](https://arxiv.org/abs/2404.18796)) finds that 3 small distinct-family judges outperform 1 large judge; same-family ensembles inherit shared training-data biases and approximate a single-judge variance regardless of N. Strategy authors targeting the bias-reduction property should populate `criticPool` with specs from at least 2 distinct providers/families. The default pool in `AGENT_CRITIC_POOL_DEFAULT` is heterogeneous; same-family overrides are accepted (no validator block) but forfeit PoLL's guarantees.
...
| **Finding-payload bounds**     | [`types.ts:FindingSchema`](./types.ts), [`parse-findings.ts`](./parse-findings.ts) | Per-field char caps + per-array length cap                                                                                      | Drop oversized fields at Zod validation; truncate findings array to `MAX_FINDINGS_PER_CRITIC` (200) by severity-desc stable sort (CRITICAL/HIGH preserved). Constants in `constants.ts`: `MAX_FINDING_CATEGORY_CHARS=200`, `MAX_FINDING_TITLE_CHARS=500`, `MAX_FINDING_DESCRIPTION_CHARS=5000`, `MAX_FINDING_FILE_CHARS=1000`, `MAX_FINDING_SUGGESTION_CHARS=5000`. |

...
**Severity aggregation**: SAST tooling conventionally uses MAX (CVSS v3.1 §3.8; GitHub/GHAS; DefectDojo). This module uses median tie-up — robust to outlier votes — with the high-severity HIGH-confidence escape hatch covering the asymmetric-cost case for `severity ∈ {HIGH, CRITICAL}`.

**Median tie-up at small N**: median tie-up favors the higher rank on ties; at N=2 this reduces to MAX (a single dissenting CRITICAL voter pulls the merged severity to CRITICAL). For a true median behavior with at least one ranked-down voter, choose `criticCount ≥ 3`.

**`disagreementScore` bound at odd N**: the score is population variance of severity ranks normalized by the theoretical max for an even-N half/half split at the extremes (`(R−1)²/4 = 9/4`). The score approaches 1 for a half-LOW/half-CRITICAL split (1 attained at even N; strictly less at odd N).

**Consensus failure**: when all critics agree on a hallucinated finding, the merged result has `disagreementScore = 0` and `votes = N` — maximum apparent confidence in the wrong answer. This kernel does not detect consensus failure; the quality ratchet and best-state restore protect against actor regressions, not critic regressions.
...
| **Per-slot escape cap**        | [`merge-findings.ts:mergeCriticFindings`](./merge-findings.ts)   | Single critic slot is the sole voter on more than `CRITIC_ESCAPE_CAP_PER_SLOT` (3) below-threshold escape-eligible groups in one merge call | Drop ALL of that slot's unilateral escape contributions before emission. Multi-voter escape groups (`voters.size ≥ 2`) unaffected.                                                                                                                                                                |

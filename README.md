...
| coherentMeterValues                                  | true/false    | false                                                                                                                              | boolean                                                                                                                                                                       | enable physics-based coherent MeterValues generation. When `true`, every emitted measurand (V, I, P, ΔE, SoC) is derived from a single physics chain enforcing `P = V·I·phases` (INV-1), `SoC(t+1) ≥ SoC(t)` (INV-2), and `ΔE = P·Δt/3.6e6` (INV-3). Requires `evProfilesFile` to be set; falls back to legacy generation with a warning if the file is absent or malformed                                                                                                |
...
| `initialSocPercentMin` | number                            | [0, 100]          | Minimum initial SoC (%) at transaction start                                                                                                         |
| `initialSocPercentMax` | number                            | [0, 100]          | Maximum initial SoC (%) at transaction start                                                                                                         |

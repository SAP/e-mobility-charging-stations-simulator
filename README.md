...
| forceTransactionOnInvalidIdToken                     | true/false    | false                                                                                                                              | boolean                                                                                                                                                                       | **NON-SPEC-COMPLIANT TEST OVERRIDE.** When `true`, station-initiated transactions continue even if the CSMS rejects the IdToken (OCPP 1.6 `idTagInfo.status` ≠ Accepted, or OCPP 2.0.1 `idTokenInfo.status` ≠ Accepted on `eventType=Started`). Mid-transaction revocation (OCPP 2.0.1 `Updated`/`Ended` event types) still triggers deauthorization regardless of this flag. Violates OCPP 2.0.1 E05.FR.09, E05.FR.10, E06.FR.04 — for testing edge-case CS implementations only. Inverts E2E expectations for P5A authorize, P5B/G19 transaction-start scenarios. |

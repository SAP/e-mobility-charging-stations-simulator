...
**Template resolution scope.** Coherent MeterValues reads `MeterValues` templates from the **connector-level** definition only. Unlike the legacy path (`getSampledValueTemplate`), EVSE-level `MeterValues` inheritance is not applied when `coherentMeterValues` is `true`. Define per-connector `MeterValues` inside each connector entry. EVSE-level inheritance under coherent mode is tracked as follow-up.

...
> **Autonomous clock-aligned MeterValues** (OCPP 2.0.1 J01): when `Enabled` is `true` and `Interval` is between 1 and 86,400 seconds, the charging station schedules on UTC-day boundaries, retains only the latest pending boundary per EVSE while a previous non-transactional request is still in flight, and skips elapsed boundaries after a delayed callback rather than backdating current measurements; `Interval = 0` disables the scheduler. While online and registered, each idle EVSE with configured `MeterValues` sends one `MeterValuesRequest` targeted by `evseId` with `ReadingContext = Sample.Clock`; each EVSE meter point aggregates its physical connectors, and station-level EVSE `0`, when its main-meter templates are configured, reports totals across all physical EVSEs. An EVSE with an ongoing transaction reports its aligned sample through `TransactionEventRequest` (`eventType = Updated`, `triggerReason = MeterValueClock`); while offline or awaiting registration these transaction events remain ordered with concurrent live events during reconnection. Each connector replay queue is capped at 10,000 events and 1 MiB of serialized data; lifecycle events displace replaceable updates under pressure. Coherent samples advance energy and SoC only for elapsed time since the last periodic or aligned sample, preventing double counting when both schedules interleave. `SignReadings = true` signs configured aligned samples when the fiscal-metering prerequisites are available. `TransactionEvent` delivery uses `MessageAttempts`, `MessageAttemptInterval`, and `MessageTimeout`; retries reuse the original payload, timestamp, and sequence number. The interval is updatable live through `SetVariables`. `SendDuringIdle = true` at station scope suppresses all aligned emissions while any transaction is ongoing; an EVSE-targeted value suppresses only that EVSE and otherwise falls back to the station-scoped value.

...
> **Autonomous clock-aligned MeterValues** (OCPP 2.0.1 J01): when `Enabled` is `true` and `Interval` > 0, the charging station autonomously emits one aggregated `MeterValuesRequest` per EVSE every `Interval` seconds with `ReadingContext = Sample.Clock`, updatable live through `SetVariables`. `SendDuringIdle = true` restricts emission to periods with no ongoing transaction. Deviations from the specification: the interval is a free-running timer (heartbeat pattern) rather than strict wall-clock/UTC boundary alignment; `SendDuringIdle` is evaluated station-scoped, so an ongoing transaction on any EVSE suppresses the sweep for all EVSEs (the registered variable carries no per-EVSE instance); no `evseId = 0` station-level request is emitted; idle readings are unsigned as they carry no transaction identity.

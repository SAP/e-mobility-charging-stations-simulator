...
- `Energy.Active.Import.Register`: no phase emits the aggregate register; `L1-N`/`L2-N`/`L3-N` emit `register / phases` (balanced 3-phase Y contribution per phase); line-to-line and `N` phases are unsupported. On OCPP 2.0.1 stations, when `SampledDataCtrlr.RegisterValuesWithoutPhases` is `true`, per-phase L-N register templates are skipped and only the aggregate register is emitted; on OCPP 1.6 stations the variable never resolves so current behavior is preserved.

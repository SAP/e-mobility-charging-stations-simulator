...
> **Autonomous clock-aligned MeterValues** (OCPP 2.0.1 J01): when `Enabled` is `true` and `Interval` is between 1 and 86,400 seconds, the charging station schedules on UTC-day boundaries, timestamps each sample when the callback actually runs, and recalculates the next boundary at every callback without suppressing a boundary while a previous request awaits its response; `Interval = 0` disables the scheduler. Idle EVSEs are aggregated into one `MeterValuesRequest` per EVSE with `ReadingContext = Sample.Clock`, while an EVSE with an ongoing transaction reports its aligned sample through `TransactionEventRequest` (`eventType = Updated`, `triggerReason = MeterValueClock`). Coherent samples advance energy and SoC only for elapsed time since the last periodic or aligned sample, preventing double counting when both schedules interleave. Transaction events are buffered if the connection closes during delivery. The interval is updatable live through `SetVariables`. `SendDuringIdle = true` restricts emission to periods with no ongoing transaction. Deviations from the specification: `SendDuringIdle` is evaluated station-scoped, so an ongoing transaction on any EVSE suppresses the sweep for all EVSEs (the registered variable carries no per-EVSE instance); no `evseId = 0` station-level request is emitted; idle aligned readings are unsigned because they have no transaction identity.

...
> **Autonomous clock-aligned MeterValues** (OCPP 2.0.1 J01): when `Enabled` is `true` and `Interval` > 0, the charging station autonomously emits one aggregated `MeterValuesRequest` per EVSE with `ReadingContext = Sample.Clock`, updatable live through `SetVariables`. The first emission is aligned to the next wall-clock boundary (intervals evenly spaced from 00:00:00 UTC, §2.7.12), then repeats every `Interval` seconds. `SendDuringIdle = true` restricts emission to periods with no ongoing transaction. Deviations from the specification: `SendDuringIdle` is evaluated station-scoped, so an ongoing transaction on any EVSE suppresses the sweep for all EVSEs (the registered variable carries no per-EVSE instance); no `evseId = 0` station-level request is emitted; idle readings are unsigned as they carry no transaction identity.

...
| `powerFactor`          | number                            | (0, 1], optional  | Optional cos φ between real and apparent power. Absent ⇒ `1` (unity, current behavior preserved). Multiplies the divisor in the per-phase current derivation `I = P / (V · phases · powerFactor)`, so `I` rises inversely proportional to `powerFactor` for a given delivered active power      |
